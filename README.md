...
| 版本 | 更新内容 |
|------|----------|
| **v2.9.6** | 回退逐条填写，支持中文数字"两"，超时延长至10分钟 |
| **v2.9.5** | 批量填写模式（已回退） |
| **v2.9.4** | 量词优先匹配，匹配不到回退纯数字 |
| **v2.9.3** | 新增中文/英文数字识别 |
| **v2.9.2** | 回到纯数字识别 |
| **v2.9.1** | 数字优先 + 量词辅助 |
| **v2.9** | 断点续填 + 自定义起始位置 + 状态持久化 |
| **v2.8.5** | 修复暂停/停止后继续执行问题 |
| **v2.8.4** | 修复量词大小写识别，状态持久化 |
| **v2.8.3** | 重构数量提取逻辑（量词优先，就近查找数字） |
| **v2.8.2** | 修复中文数字识别，新增 501 阈值判断 |
| **v2.8.1** | 优化数量识别阈值（501），修复悬浮窗加载 |
| **v2.8** | 新增数量 > 501 不参与计算规则 |
| **v2.7** | 修复纯数字被误识别为数量的问题 |
| **v2.6** | 新增动态倍率、悬浮窗拖拽、日志持久化 |
| **v2.5** | 新增变种自动识别、智能修正 |
| **v2.0** | 支持多规格（变种）商品 |
| **v1.0** | 基础批量填写功能 |
