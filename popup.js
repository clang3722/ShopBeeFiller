...
let isPanelOpen = true; // popup 默认打开
...
function saveSettings() {
  const settings = {
    ...
    floatEnabled: floatToggle.checked
  };
...
function loadSettings() {
  chrome.storage.local.get(['shopeeSettings'], (result) => {
    if (result.shopeeSettings) {
      ...
      if (settings.floatEnabled !== undefined) floatToggle.checked = settings.floatEnabled;
    }
  });
}
...
floatToggle.addEventListener('change', saveSettings);
...
function log(message, type = 'info') {
  if (!isPanelOpen) return; // 面板关闭时不记录
...
async function injectFillScript(rows, rate, interval, threshold) {
  ...
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => {
      if (!isResolved) {
        isResolved = true;
        resolve({ success: false, message: '操作超时（60秒），请刷新页面后重试' });
      }
    }, 60000);
...
fileInput.addEventListener('change', () => {
  if (fileInput.files && fileInput.files[0]) {
    ...
    chrome.storage.local.set({ lastFile: fileInput.files[0].name });
  }
});
...
log('💡 所有设置自动保存，悬浮窗可在店小蜜页面使用', 'info');